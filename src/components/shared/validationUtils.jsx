...
export const validateForm = (formData, validationRules) => {
    ...
    Object.keys(validationRules).forEach(field => {
      ...
      if (rules.pattern && formData[field] && !rules.pattern.test(formData[field])) {
        errors[field] = rules.patternMessage || `Invalid ${field} format`;
      }
      
      // Check min length
      if (rules.minLength && formData[field] && formData[field].length < rules.minLength) {
        errors[field] = rules.minLengthMessage || `${field} must be at least ${rules.minLength} characters`;
      }
      
      // Check max length
      if (rules.maxLength && formData[field] && formData[field].length > rules.maxLength) {
  ...
  export const validationPatterns = {
    ...
    phone: {
      // Updated pattern to better handle international formats with varying country code lengths
    ...
    url: {
      ...
      message: 'Please enter a valid URL starting with http:// or https:// (e.g., https://example.com)'
    }
  };
