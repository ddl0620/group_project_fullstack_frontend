...
import { Calendar, MapPin, Clock, Users, CheckCircle, XCircle, HelpCircle, ArrowLeft } from "lucide-react"
...
export default function MyInvitations() {
      ...
      try {
        ...
        if (response.success) {
          const userInvitations = response.content.events.filter(event =>
            event.participants.some(p => p.userId === user._id && p.status === "INVITED")
          )
              ...
              try {
                ...
                if (organizerResponse.success) {
                  return {
                    ...
                    organizer: {
                      _id: event.organizer,
                      name: organizerResponse.content.name || "Unknown Organizer",
                ...
                console.error(`Failed to fetch organizer ${event.organizer}:`, error)
                return {
                  ...event,
                  organizer: {
                    _id: event.organizer,
  ...
  const filteredInvitations = invitations.filter((event) => {
    const status = event.participants.find((p) => p.userId === user._id)?.status
  ...
  const respondToInvitation = async (eventId, response) => {
    try {
      ...
      Toast.success(`You have ${response === "accept" ? "accepted" : "declined"} the invitation`)
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">My Event Invitations</h1>
          <p className="mt-2 text-gray-600">Manage invitations you've received to participate in events</p>
        </div>
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                {paginatedInvitations.map((event) => {
                  const participantInfo = event.participants.find((p) => p.userId === user._id)
                          ...
                          <CardFooter className="border-t bg-gray-50 p-4">
                            <div className="flex w-full items-center justify-between">
                              <p className="text-xs text-gray-500">
                                Invited on {format(parseISO(participantInfo.invitedAt), "MMM d, yyyy")}
                              </p>

                              {isPending ? (
                                <div className="flex gap-2">
                                  <Button
                                    onClick={() => respondToInvitation(event._id, "decline")}
                                    variant="outline"
                                  ...
                                  <Button
                                    onClick={() => respondToInvitation(event._id, "accept")}
                                ...
                                <Button onClick={() => navigate(`/event/${event._id}`)} variant="outline">
                                  View Event
                                </Button>
                              )}
                            </div>
