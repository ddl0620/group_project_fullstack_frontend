...
export default function EventDetailsModal({ isOpen, setIsOpen, event }) {
  ...
  const [isParticipantsFilterChanged, setIsParticipantsFilterChanged] =
    useState(false);
  ...
  const acceptedParticipants =
    displayEvent.participants?.filter((p) => p.status === 'ACCEPTED') || [];
  const pendingParticipants =
    displayEvent.participants?.filter((p) => p.status === 'PENDING') || [];
  const declinedParticipants =
    displayEvent.participants?.filter((p) => p.status === 'DENIED') || [];

  // Filter participants based on selected filter
  const filteredParticipants =
    participantFilter === 'all'
      ? displayEvent.participants || []
      : displayEvent.participants?.filter(
          ...
          <TabsContent value="details" className="mt-4 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div className="mb-2 sm:mb-0">
                  <Badge className="mb-2">
                    {getEventTypeDisplay(displayEvent.type)}
                  </Badge>
              ...
              <div>
                ...
                <div className="flex items-center">
                  <Avatar className="mr-2 h-8 w-8">
                    ...
                    />
                    <AvatarFallback>
                      {getInitials(displayEvent.organizer?.name || 'Organizer')}
                    </AvatarFallback>
                  </Avatar>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="rounded-lg border p-3">
                ...
                <p className="text-muted-foreground text-sm">Going</p>
              </div>
              <div className="rounded-lg border p-3">
          ...
          <TabsContent value="participants" className="mt-4">
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex space-x-2">
                <Button
                  variant={
                    participantFilter === 'ACCEPTED' ? 'default' : 'outline'
                  }
                  size="sm"
                ...
                <Button
                  variant={
                    participantFilter === 'PENDING' ? 'default' : 'outline'
                  }
                  size="sm"
                      ...
                      >
                        <div className="flex items-center">
                          <Avatar className="mr-3 h-10 w-10">
                            <AvatarImage
                              src={
                                participant.userId.avatar || '/placeholder.svg'
                              }
                              alt={participant.userId.name || 'User'}
                            />
                            <AvatarFallback>
                              {getInitials(participant.userId.name || 'User')}
                            </AvatarFallback>
                          </Avatar>
                          <div>
