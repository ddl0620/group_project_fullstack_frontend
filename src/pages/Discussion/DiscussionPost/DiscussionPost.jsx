...
export function DiscussionPost({ postData }) {
  ...
  const repliesByPostId = useSelector((state) => state.discussionReply.repliesByPostId)
  ...
  return (
    <div>
      <Card className="mx-auto max-w-2xl rounded-lg bg-white shadow-md w-full">
        <CardHeader className="flex flex-row items-center gap-2 sm:gap-4 space-y-0 p-2 sm:p-4">
          <CustomAvatar
            src={creator.avatar}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1">
              <span className="font-semibold text-sm sm:text-base truncate">{creator?.name || "User"}</span>
            </div>
          ...
          {isMyPost && (
            <div className="flex gap-1">
              <CreateEditDiscussionPost
                ...
                triggerButton={
                  <Button size={"sm"} variant={"ghost"} className="h-7 w-7 p-0 sm:h-8 sm:w-8 sm:p-1">
        ...
        <CardContent className="flex flex-col items-start justify-center p-2 sm:p-4 pt-0 sm:pt-0 w-full">
          ...
          {postData.images && postData.images.length > 0 ? (
            ...
            <div className="w-full mt-2 min-h-[1rem]"></div>
          )}
        </CardContent>
        <CardFooter className="flex flex-col p-0">
          <div className="flex w-full items-center justify-start gap-x-2 sm:gap-x-4 p-2 sm:p-4">
            <Button
              variant="ghost"
              size="sm"
              className="flex items-center gap-1 h-7 sm:h-9 px-2 sm:px-3 text-xs sm:text-sm"
          ...
          {showComments && <CommentSection postId={postData._id} initialReplies={replies} />}
        </CardFooter>
      </Card>
    </div>
  )
