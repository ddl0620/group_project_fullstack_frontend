...
export const useUserStatis = () => {
  ...
  const engagementStats = useSelector(
    (state) => state.userStatis.engagementStats
  );
  const invitationsOverTime = useSelector(
    (state) => state.userStatis.invitationsOverTime
  );
  const rsvpTrend = useSelector((state) => state.userStatis.rsvpTrend);
  const rsvpDistribution = useSelector(
    (state) => state.userStatis.rsvpDistribution
  );
  const recipients = useSelector((state) => state.userStatis.recipients);
  const totalRecipients = useSelector(
    (state) => state.userStatis.totalRecipients
  );
  ...
  const fetchEngagementStats = useCallback(
    async (params = {}, forceRefresh = false) => {
      ...
      if (
        !forceRefresh &&
        cache.current.engagementStats &&
        cache.current.engagementStats.cacheKey === cacheKey
      ) {
  ...
  const fetchInvitationsOverTime = useCallback(
    async (params, forceRefresh = false) => {
      ...
      if (!forceRefresh && cache.current.invitationsOverTime[cacheKey]) {
        ...
        dispatch(
          setInvitationsOverTime(cache.current.invitationsOverTime[cacheKey])
        );
  ...
  const fetchRsvpDistribution = useCallback(
    async (params = {}, forceRefresh = false) => {
      ...
      if (
        !forceRefresh &&
        cache.current.rsvpDistribution &&
        cache.current.rsvpDistribution.cacheKey === cacheKey
      ) {
