...
export const useAdminManagement = () => {
  ...
  const userActivities = useSelector(
    (state) => state.adminManagement.userActivities
  );
  const eventDetails = useSelector(
    (state) => state.adminManagement.eventDetails
  );
  ...
  const fetchUserActivities = useCallback(
    async (userId, page = 1, limit = 10, isAcs = true) => {
      ...
      try {
        ...
        dispatch(setLoading(true));
        checkToken();
        const response = await getAllEventsByUserIdAPI(
          userId,
          page,
          limit,
          isAcs
        ...
        if (!response.success) {
          throw new Error(
            response.message || 'Failed to fetch user activities'
          );
        }
        const events = Array.isArray(response.content.events)
  ...
  const fetchEvents = useCallback(
    async (page = 1, limit = 10, isAcs = true, forceRefresh = false) => {
      ...
      try {
        ...
        const result = {
          events: Array.isArray(response.content.events)
            ? response.content.events
            : [],
          pagination: response.content.pagination || {
            page,
            limit,
            total: response.content.events.length,
          },
        };
  ...
  const updateActiveStatus = useCallback(
    async (eventId, data) => {
      ...
      try {
        ...
        dispatch(setLoading(true));
        checkToken();
        const response = await updateActiveStatusAPI(eventId, data);
        if (!response.success) {
          throw new Error(
            response.message || 'Failed to update active status event'
          );
        }
        ...
        if (data.isDeleted) {
          clearCache('events-');
        } else dispatch(updateEvent(response.content.event));

        Toast.success("Event's status updated successfully");
        return response.content;
      } catch (error) {
        const errorMessage = error.response?.data?.message || error.message;
        dispatch(setError(errorMessage));
